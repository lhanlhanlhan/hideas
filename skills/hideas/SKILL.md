...
- Inspecting version or build-time information for the local binary or the connected server.
- Inspecting the configured Hideas server or login state.
...
The `hideas` CLI is always a client. It cannot operate without a configured server and a valid bearer token. The CLI writes the active `server` into `~/.hideas/config` after a successful `hideas login`.
...
3. Use `hideas version` to inspect the local binary or the connected server version.
4. Use `hideas status` to inspect the configured server and login state.
...
9. Before running data commands, check `hideas auth status` to ensure the client is authenticated.
10. If not authenticated, run `hideas login --server URL` and open the printed authorization URL. Use `--wait` only when the agent can block until the browser flow finishes.
